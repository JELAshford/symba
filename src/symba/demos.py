...
for name, rule in rules.items():
    ...
    save_state(this_grid, Path(f"out/demos/{name}.png"), cmap="bwr")
...
for step in range(1, timesteps):
    ...
    grid[:, :20] = 0
    grid[:, -20:] = 0
save_state(grid, Path("out/demos/fig17.png"))
...
for step in range(1, timesteps):
    ...
    grid[:, :20] = 0
    grid[:, -20:] = 0
save_state(grid, Path("out/demos/fig5.png"))
